...
self.addEventListener("push", (event) => {
  ...
  const title = payload.title ?? "Odi.Pet \u{1F43E}";
  const body = payload.body ?? "Yeni bir bildiriminiz var.";
  const notificationTag = payload.tag ?? (payload.entity_type && payload.entity_id ? `${payload.entity_type}:${payload.entity_id}:${payload.event ?? "due"}` : `odi-${Date.now()}`);
  const options = {
    body,
    ...
    tag: notificationTag,
    renotify: false,
    // Prevents lock screen cluttering for same task
    data: {
