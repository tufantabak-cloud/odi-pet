...
export default function FloatingLostPets({ userCities }: { userCities: string[] }) {
  ...
  useEffect(() => {
    ...
    // We only fetch active reports where the pet's city matches one of the user's cities
    supabase
      .from('lost_reports')
  ...
  const hasActiveReports = lostReports.length > 0;
        ...
        {hasActiveReports ? (
          <div className="flex flex-col gap-4 max-h-[60vh] overflow-y-auto scrollbar-none pb-2">
            {lostReports.map((report) => (
              <div key={report.id} className="bg-warning/5 border border-warning/20 rounded-[20px] p-4 flex flex-col gap-3 relative shadow-sm">
                ...
                {report.contact_phone && (
                  <a href={`tel:${report.contact_phone}`} className="w-full bg-warning text-white font-bold text-[13px] rounded-xl py-2.5 text-center flex items-center justify-center gap-2 hover:bg-warning/90 active:scale-[0.98] transition-all">
          ...
          <div className="p-6 bg-surface border border-border-main rounded-2xl flex flex-col items-center justify-center text-center">
            <div className="w-12 h-12 rounded-full bg-success/10 flex items-center justify-center mb-3">
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" className="text-success"><path d="M20 6L9 17l-5-5"/></svg>
            </div>
            <p className="text-[14px] font-bold text-text-primary mb-1">Harika Haber!</p>
            <p className="text-[12px] text-text-secondary">Şu an şehrinizde aktif bir kayıp ilanı bulunmuyor.</p>
          </div>
        )}

        <button onClick={() => setOpen(false)} className="btn-secondary w-full mt-4 py-3 text-[14px]">Kapat</button>
      </div>
    </div>
  )
