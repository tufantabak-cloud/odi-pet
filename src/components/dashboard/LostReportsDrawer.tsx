          ...
          <div className="relative flex flex-col w-full max-w-[380px] h-full bg-white shadow-2xl animate-in slide-in-from-right duration-300">
            ...
            <div className="flex items-center justify-between p-4 border-b border-gray-100">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h3 className="font-extrabold text-[15px] text-gray-900 leading-none">Bölgendeki Kayıp İlanları</h3>
                  <span className="text-[10px] text-gray-500 font-semibold mt-1 block">Bulunduğun şehirde aktif arama ilanları</span>
                </div>
              </div>
              <button
                onClick={() => setIsOpen(false)}
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3 scrollbar-none">
              {reports.map((report) => (
                <div
                  key={report.id}
                  className="bg-red-50/40 border border-red-100 rounded-xl p-3 flex flex-col gap-2.5 shadow-sm hover:border-red-200 transition-colors"
                >
                  <div className="flex items-center gap-3">
                    <div className="w-12 h-12 relative rounded-lg overflow-hidden border border-red-200 bg-white shrink-0">
                      {report.pets?.avatar_url ? (
                        ...
                        <div className="w-full h-full flex items-center justify-center bg-gray-50 text-red-400 font-bold text-sm">
                          {report.pets?.name?.charAt(0) || '?'}
                        </div>
                      )}
                    </div>
                    <div className="min-w-0">
                      <h4 className="font-black text-sm text-gray-900 truncate">{report.pets?.name}</h4>
                  ...
                  <div className="bg-white rounded-lg p-2 border border-red-50 flex items-start gap-1.5">
                    ...
                    <div>
                      <p className="text-[9px] text-gray-400 font-bold uppercase tracking-wider">Son Görülme</p>
                      <p className="text-[11px] font-semibold text-gray-800 leading-tight mt-0.5">
                  ...
                  <a
                    ...
                    className="w-full h-10 bg-red-600 hover:bg-red-700 active:scale-[0.98] text-white font-extrabold text-xs rounded-lg flex items-center justify-center gap-1.5 transition-all shadow-md shadow-red-600/10"
                  >
