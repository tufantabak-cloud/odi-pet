...
export default function WeatherPawAlert({ activePet }: WeatherPawAlertProps) {
  ...
  // Asfalt sıcaklığı tahmini (T_air * 1.6 civarı)
  const asphaltTemp = Math.round(weather.temp * 1.6);
  
  const isDanger = weather.temp >= 30; // 30+ hava -> ~48+ asfalt (Tehlikeli)
  const isCaution = weather.temp >= 25 && weather.temp < 30; // 25-30 hava -> ~40-48 asfalt (Dikkat)
  const isSafe = weather.temp < 25; // <25 hava -> Güvenli
      ...
      <div className="pr-5">
        ...
        <p className={`text-xs mt-1 leading-relaxed ${textDescStyle}`}>
          {isSafe ? (
            ...
            <>Dışarısı <strong>{Math.round(weather.temp)}°C</strong>, asfalt sıcaklığı yaklaşık <strong>{asphaltTemp}°C</strong>'yi bulabilir. Yürüyüş için {isDanger ? 'patileri koruyan patik giydirin ya da' : 'mümkünse çim alanları tercih edin veya'} <strong>{sunsetStr}</strong> sonrasını bekleyin.</>
          )}
        </p>
      </div>
    </div>
