...
export default function LogbookSheet({ petId, onClose }: LogbookSheetProps) {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[10000] flex flex-col justify-end">
      ...
      {/* Sheet Content */}
      <div className="relative w-full max-w-lg mx-auto bg-surface rounded-t-[30px] shadow-2xl animate-in slide-in-from-bottom-full duration-300 pb-8 flex flex-col max-h-[85vh]">
        ...
        <div className="px-6 pb-2 shrink-0 flex items-center justify-between">
          <h2 className="text-[20px] font-extrabold text-text-primary">Durum Kaydet</h2>
          <button onClick={onClose} className="w-8 h-8 flex items-center justify-center rounded-full bg-bg-main text-text-secondary hover:text-primary transition-colors">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M18 6 6 18M6 6l12 12"/></svg>
          </button>
        </div>

        <div className="px-6 overflow-y-auto pb-4 flex flex-col gap-3 scrollbar-none">
          <p className="text-[14px] text-text-secondary mb-1">Hangi durumu kaydetmek istiyorsunuz?</p>
          
          {CATEGORIES.map(cat => (
            <Link 
              key={cat.id} 
              href={`/owner/pets/${petId}/journal/new/${cat.id}`} 
              className="card-base p-4 flex items-center gap-4 border border-border-main hover:border-primary/40 hover:shadow-md transition-all group bg-white"
            >
              ...
              <div className="flex-1 min-w-0">
                <h3 className="text-[16px] font-extrabold text-text-primary group-hover:text-primary transition-colors">{cat.label}</h3>
                <p className="text-[13px] text-text-secondary truncate">{cat.desc}</p>
              </div>
