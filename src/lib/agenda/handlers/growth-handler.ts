...
export class GrowthMeasurementReadHandler implements AgendaReadHandler {
  ...
  normalizePlan(plan: any, context: AgendaNormalizationContext): PetAgendaEvent {
    ...
    return {
      ...
      displayStatus: plan.status === 'completed' ? 'completed' : dateKey < context.todayStr ? 'overdue' : dateKey === context.todayStr ? 'today' : 'upcoming',
      status: plan.status === 'completed' ? 'completed' : dateKey < context.todayStr ? 'overdue' : dateKey === context.todayStr ? 'today' : 'upcoming',
      repeatRule: null,
      isVirtual: false,
