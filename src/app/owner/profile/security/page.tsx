      ...
      <main className="max-w-xl w-full mx-auto p-4 sm:p-6 flex flex-col gap-6">
        ...
        <section className="bg-surface rounded-3xl p-6 border border-border-main shadow-[0_4px_20px_-2px_rgba(15,23,42,0.04)] flex flex-col gap-4">
          <div className="flex items-center gap-2.5 pb-2 border-b border-border-main/60">
            ...
            <h2 className="text-sm font-bold text-text-primary uppercase tracking-wider">Hesap Verilerini Dışa Aktar</h2>
          </div>
          <p className="text-xs text-text-secondary leading-relaxed">
            Hesabınızdaki tüm verileri (evcil hayvanlar, aşılar, sağlık kayıtları, planlar) JSON formatında indirebilirsiniz. Oturumunuz aktif kalmaya devam eder.
          </p>
          <a
            href="/api/owner/export-data"
            download="odi-personal-data.json"
            data-testid="export-account-data"
            className="w-full min-h-11 py-3 px-4 rounded-2xl bg-surface border border-primary/30 hover:bg-primary/5 text-primary text-xs font-bold transition-all shadow-xs active:scale-[0.98] flex items-center justify-center gap-2"
          >
            <Download className="w-4 h-4" />
            <span>Hesap Verilerini Dışa Aktar (JSON)</span>
          </a>
        </section>
      </main>
    </div>
