...
export default function TreatmentsClient({ pet }: { pet: any }) {
  ...
  if (loading) {
    return <div className="p-10 text-center text-text-secondary text-[14px]">Sağlık planları yükleniyor...</div>
  }
    ...
    <div className="flex flex-col gap-6 pb-20 w-full mx-auto animate-fadeIn">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 relative">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-[24px] font-black text-text-primary">Sağlık Takip Modülü</h1>
            <p className="text-[14px] text-text-secondary font-medium">{pet.name} için hastalık ve tedavi geçmişi</p>
          </div>
        </div>
      </div>

      <div className="card-base p-5 border-l-4 border-l-primary flex flex-col gap-3">
        <p className="text-[14px] text-text-secondary font-medium">Yeni ilaç tedavisi veya sağlık planı eklemek için ana sayfadaki veya menüdeki <b>Plan Yap</b> sihirbazını kullanın.</p>
        <button onClick={() => router.push(`/owner/plan-yap/saglik?pet_id=${pet.id}`)} className="btn-primary w-max py-2 px-4 shadow-sm">
      ...
      <div>
        ...
        {activePlans.length === 0 ? (
          <div className="text-[14px] text-text-secondary p-4 bg-white rounded-xl border border-border-main">Aktif sağlık planı bulunmuyor.</div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {activePlans.map(plan => (
              <div key={plan.id} className="card-base p-5 flex flex-col gap-2">
                ...
                {plan.note && <p className="text-[12px] text-text-secondary mt-1"><b>Not:</b> {plan.note}</p>}
              </div>
            ))}
          </div>
        )}
