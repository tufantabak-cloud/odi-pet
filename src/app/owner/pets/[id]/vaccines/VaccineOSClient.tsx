...
function ParasiteTable({ pet, templates, records, onCellClick, onNewRecord, onNewPlan, onBatchScan, wizardDismissed = true }: { 
  ...
  const rows = parasiteTemplates.map(tmpl => {
    ...
    const completedRecords = allRecords
      .filter(r => r.status === 'completed')
      .sort((a, b) => new Date(b.administered_at || b.due_at || '').getTime() - new Date(a.administered_at || a.due_at || '').getTime());
    ...
    let refDate = completedRecords.length > 0
      ? new Date(completedRecords[0].administered_at || completedRecords[0].due_at || new Date().toISOString())
    ...
    for (let i = 0; i < 5; i++) {
      ...
      const matchedCompleted = completedRecords.find(r => {
        if (r.id === firstCompleted?.id) return false;
