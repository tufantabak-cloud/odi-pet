...
function ParasiteTable({ pet, templates, records, onCellClick, onNewRecord, onNewPlan, onBatchScan, wizardDismissed = true }: { 
  ...
  const rows = parasiteTemplates.map(tmpl => {
    ...
    const completedRecords = allRecords
      .filter(r => r.status === 'completed')
      .sort((a, b) => new Date(b.administered_at || b.due_at || '').getTime() - new Date(a.administered_at || a.due_at || '').getTime()); // YENİ -> ESKİ
    ...
    // ── Hücre 0: İlk Uygulama (Kesinlikle Tamamlanmış veya Kayıt Gir) ──
    ...
    for (let i = 0; i < 5; i++) {
      ...
      if (matchedPending) {
        ...
        refDate = projectedDate; // Sapma yok, projected tarihle devam
      }
    }
