...
// GET — ürün önerilerini durum filtresiyle listeler (admin)
export async function GET(req: NextRequest) {
  ...
  if (!['pending', 'approved', 'rejected', 'merged', 'all'].includes(status)) {
  ...
  let query = supabase
    ...
    .select('*, profiles!parasite_product_suggestions_suggested_by_fkey(full_name, email)')
