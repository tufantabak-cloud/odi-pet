    ...
    <div className="flex min-h-screen w-full items-center justify-center p-4 bg-bg-main bg-gradient-to-tr from-primary/5 via-transparent to-primary/5">
      <div className="w-full max-w-[420px] card-base p-8 sm:p-10 animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        <form className="flex flex-col gap-6" onSubmit={handleSubmit(onSubmit)}>
          ...
          />
          {error && (
            <div role="alert" className="p-4 rounded-2xl bg-error/10 border border-error/20 text-error text-[13px] font-bold text-center animate-in shake-in duration-300">
              ⚠️ {error}
            </div>
          )}

          <fieldset disabled={loading || googleLoading} className="flex flex-col gap-6">
          ...
          <div className="flex flex-col gap-4">
            <button
              ...
              className="btn-base w-full py-3.5 bg-white border border-border-main text-text-primary font-bold shadow-sm hover:bg-bg-subtle hover:scale-[1.02] active:scale-[0.98] transition-all flex items-center justify-center gap-3 disabled:opacity-60 rounded-[14px]"
            >
              {googleLoading ? (
          ...
          <div className="flex flex-col gap-1.5">
            ...
            {passwordValue && strength.text && (
              <div className={`text-[10px] font-bold mt-0.5 text-right px-1 ${strength.color.replace('bg-', 'text-')}`}>
                {strength.text}
              </div>
            )}
