-- Harden consume_feature_usage: user_subscriptions.plan serbest TEXT ve uzerinde
-- CHECK constraint yok. Beklenmedik bir deger (orn. 'premium', 'trial') plan_tier_enum
-- cast'inde exception firlatir ve RPC komple patlar. Cast'i beyaz listeye baglariz.
-- Islevsel degisiklik yok: ai_plus_until / pro_until zaten oncelikli okunuyor.
...
BEGIN
  ...
  IF FOUND THEN
    ...
    ELSIF v_sub.pro_until IS NOT NULL AND v_sub.pro_until > now() THEN
      v_user_plan := 'pro';
    ELSIF v_sub.plan IN ('pro', 'ai_plus') THEN
