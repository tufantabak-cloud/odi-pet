-- Migration: 20260813100002_atomic_completion_and_notification_cleanup
-- Description: Updates complete_recurring_plan RPC to atomically delete pending notification_jobs for the completed occurrence.
...
BEGIN
  ...
  -- 3. Idempotency Check FIRST (before occurrence matching check!)
  -- If a completed child row for this parent_plan_id & occurrence_scheduled_at already exists, return idempotent response!
  ...
  -- 4. OCCURRENCE VALIDATION: p_occurrence_scheduled_at MUST match current v_plan.scheduled_at
  IF p_occurrence_scheduled_at <> v_plan.scheduled_at THEN
    RAISE EXCEPTION 'INVALID_OCCURRENCE';
  END IF;

  -- 5. NEXT SCHEDULED AT VALIDATION: p_next_scheduled_at MUST be > p_occurrence_scheduled_at
  ...
  -- 8. ATOMIC NOTIFICATION CLEANUP (OPOS v2.0 Architecture Fix)
  -- Delete any pending notification jobs for this plan that are tied to this occurrence or later.
  -- Since the user completed the occurrence (even if early), we don't want to alert them about it.
...
-- Security Hardening: Revoke from PUBLIC, anon, authenticated and grant to service_role ONLY
